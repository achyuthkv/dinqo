...
export class Polls {
  ...
  pendingFor(playerId: string, communityId: string): Row[] {
    return this.pollableEvents(communityId).filter((e) => {
      const reg = this.db.get('SELECT status FROM registrations WHERE event_id = ? AND player_id = ?', e.id, playerId);
      if (reg && ['waitlisted', 'offered', 'held', 'confirmed', 'cancelled'].includes(reg.status)) return false;
      const inv = this.db.get('SELECT status FROM invitations WHERE event_id = ? AND player_id = ?', e.id, playerId);
  ...
  run(communityId: string): { poll_id: string | null; events: number; sent: number; skipped: number } {
    ...
    for (const p of regulars) {
      const pending = this.pendingFor(p.id, communityId);
  ...
  listMessage(pending: Row[], communityName: string, playerName?: string | null, followUp = false) {
    const rows: ListRow[] = pending.slice(0, 9).map((e) => {
      const left = e.capacity - this.booking.seatsTaken(e.id);
      return {
